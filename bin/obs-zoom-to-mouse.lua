...
function get_monitor_info(source)
	...
	if not info then
		log(
			"WARNING: Could not auto calculate zoom source position and size.\n"
...
function refresh_sceneitem(find_newest)
	...
	if is_non_display_capture then
		if not use_monitor_override then
			log(
				"ERROR: Selected Zoom Source is not a display capture source.\n"
	...
	if sceneitem ~= nil then
		...
		if source_width == 0 or source_height == 0 then
			log(
				"ERROR: Something went wrong determining source size."
					.. "       Try using the 'Set manual source position' option and adding override values"
			)
		...
		if sceneitem_info.bounds_type == obs.OBS_BOUNDS_NONE then
			...
			log(
				"WARNING: Found existing non-boundingbox transform. This may cause issues with zooming.\n"
					...
					if name ~= CROP_FILTER_NAME and name ~= "temp_" .. CROP_FILTER_NAME then
						...
						if settings ~= nil then
							if not obs.obs_data_get_bool(settings, "relative") then
								zoom_info.source_crop_filter.x = zoom_info.source_crop_filter.x
									+ obs.obs_data_get_int(settings, "left")
								zoom_info.source_crop_filter.y = zoom_info.source_crop_filter.y
									+ obs.obs_data_get_int(settings, "top")
								zoom_info.source_crop_filter.w = zoom_info.source_crop_filter.w
									+ obs.obs_data_get_int(settings, "cx")
								zoom_info.source_crop_filter.h = zoom_info.source_crop_filter.h
									+ obs.obs_data_get_int(settings, "cy")
								log(
									"Found existing relative crop/pad filter ("
										.. name
										.. "). Applying settings "
								...
								log(
									"WARNING: Found existing non-relative crop/pad filter ("
										.. name
										.. ").\n"
		...
		if
			not found_crop_filter
			and (
				sceneitem_crop_orig.left ~= 0
				or sceneitem_crop_orig.top ~= 0
				or sceneitem_crop_orig.right ~= 0
				or sceneitem_crop_orig.bottom ~= 0
			)
		then
			...
			log(
				"WARNING: Found existing transform crop. This may cause issues with zooming.\n"
		...
		if zoom_state == ZoomState.ZoomingOut or zoom_state == ZoomState.ZoomingIn then
			...
			if is_following_mouse then
				...
				if not use_follow_outside_bounds then
					if
						zoom_target.raw_center.x < zoom_target.crop.x
						or zoom_target.raw_center.x > zoom_target.crop.x + zoom_target.crop.w
						or zoom_target.raw_center.y < zoom_target.crop.y
						or zoom_target.raw_center.y > zoom_target.crop.y + zoom_target.crop.h
					then
				...
				if not skip_frame then
					...
					if
						locked_center == nil
						and (zoom_target.crop.x ~= crop_filter_info.x or zoom_target.crop.y ~= crop_filter_info.y)
					then
						...
						if is_following_mouse and locked_center == nil and locked_last_pos ~= nil then
							...
							if math.abs(locked_last_pos.diff_x) > math.abs(locked_last_pos.diff_y) then
								if
									(diff.auto_x < 0 and locked_last_pos.diff_x > 0)
									or (diff.auto_x > 0 and locked_last_pos.diff_x < 0)
								then
									lock = true
								end
							else
								if
									(diff.auto_y < 0 and locked_last_pos.diff_y > 0)
									or (diff.auto_y > 0 and locked_last_pos.diff_y < 0)
								then
									lock = true
								end
							end
							...
							if
								(lock and use_follow_auto_lock)
								or (diff.x <= follow_safezone_sensitivity and diff.y <= follow_safezone_sensitivity)
							then
...
function on_print_help()
	local help = "\n----------------------------------------------------\n"
		.. "Help Information for OBS-Zoom-To-Mouse v"
		.. VERSION
		.. "\n"
...
function script_properties()
	...
	local sources_list = obs.obs_properties_add_list(
		props,
		"source",
		"Zoom Source",
	...
	obs.obs_property_set_long_description(
		allow_all,
		"Enable to allow selecting any source as the Zoom Source\n"
...
function script_load(settings)
	...
	hotkey_follow_id =
		obs.obs_hotkey_register_frontend("toggle_follow_hotkey", "Toggle follow mouse during zoom", on_toggle_follow)
...
function script_update(settings)
	...
	if
		...
		or use_monitor_override ~= old_override
		or monitor_override_x ~= old_x
		or monitor_override_y ~= old_y
		or monitor_override_w ~= old_w
		or monitor_override_h ~= old_h
		or monitor_override_sx ~= old_sx
		or monitor_override_sy ~= old_sy
		or monitor_override_w ~= old_dw
		or monitor_override_h ~= old_dh
	then
		monitor_info = get_monitor_info(source)
	end
end
